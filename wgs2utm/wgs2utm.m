function [x,y,utmzone1,utmhemi1] = wgs2utm(Lon,Lat,utmzone,utmhemi,Lon0,Lat0)

if ~sum(double(nargin==[2,4,6]))
    error('Wrong number of input arguments');return
end
[x,y,utmzone1,utmhemi1] = wgs2utm_c(Lon,Lat,utmzone,utmhemi);
if exist('Lat0','var') && exist('Lon0','var')
    [x0,y0,~,~] = wgs2utm_c(Lon0,Lat0,utmzone,utmhemi);
     x=x-x0;
    y=y-y0;
end
...
function  [x,y,utmzone,utmhemi] = wgs2utm_c(Lon,Lat,utmzone,utmhemi)
% -------------------------------------------------------------------------
