...
grdgradient $bathy -A0 -Nt -Ggradients.grd
...
for center in ${centers[@]} ; do
	...
	rm -f track.xy
	psxy tmp/box.xy -Jm$scalem -R$range -A -m -O -K -L >> map.ps
...
for filename in ${files[@]} ; do
	...
	for center in ${centers[@]} ; do
		...
		###### Plot Grid projection if any
			...
			psxy tmp/new_count_proj2.txt $range_p -W1p,black -O -K -m >> $profile_name
		...
		#### Plot epicenters if any
			...
			num_col=$( awk '{print NF}' tmp/data.pzm | tail -n 1)
			
			if [[ $num_col -eq 4 ]];then
			echo 'hello'
				awk -v scale_mag=$scale_mag '{print $1,$2,$4,scale_mag*2.5^$3}' tmp/data.pzm | psxy $range_p ${eq_attributes[$eq_counter]} -K -O >> $profile_name
			fi
...
if [ $intensity == "0" ];then
    ...
    grdcontour $bathy -Jm$scalem -R$range -W0.3p -C0.1 -L0.09/0.11 -S1 -K -O >> map.ps
fi
...
for station in ${stations[@]} ; do
	...
	awk '{print $1+0.05,$2,8,0,1,"LB",$4}' $station | pstext -Jm$scalem -R$range -O -K >> map.ps
done
