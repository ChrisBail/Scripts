...
def usage():
   ...
   print "     python ntk_computePSD.py     param=computePSD  TA 959A EP   2013-10-01T11:00:00 2013-10-01T12:00:00 period     1"
   print " "
   print "\n\nOUTPUT:\n\n"
...
if(len(inStation) <= 2):
   if msgLib.param(param,'requestClient').requestClient == "IRIS":
...
for tStep in xrange(0,duration,msgLib.param(param,'windowShift').windowShift):
   ...
   try:
      ...
      #
      # read from files but request response via WS 
      #
      elif msgLib.param(param,'requestClient').requestClient == "FILES":
