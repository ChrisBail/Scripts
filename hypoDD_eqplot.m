...
file_sta='hypoDD.sta'; 
file_line='';
phiA=0; xshift=0.; yshift=0.; box_l=100.0; box_w=50;
axlim=50; minmag=5; id=[]; itake=[]; err=1;
...
figure(1); clf; set(gcf,'clipping','on');
...
if(err==1); for i=1:length(x)
	hold on 
	plot3([x(i)-ex(i) x(i)+ex(i)],[y(i) y(i)],[z(i) z(i)],'color','r');
	plot3([x(i) x(i)],[y(i)-ey(i) y(i)+ey(i)],[z(i) z(i)],'color','r');
	plot3([x(i) x(i)],[y(i) y(i)],[z(i)-ez(i) z(i)+ez(i)],'color','r');
end; end
if(length(id)>0); for i=1:length(x)
	hold on 
	for k=1:length(id); if(id(k)==cusp(i)); 
		plot3(x(i),y(i),z(i),'o','markersize',10,'color','g'); end;end;
end;end;
plot3(x(find(mag>minmag)),y(find(mag>minmag)),z(find(mag>minmag)),'o','color','r');
axis('equal'); axis([-axlim axlim -axlim axlim -2*axlim 0]); set(gca,'box','on');
title('3D VIEW'); xlabel('distance [km]'); ylabel('distance [km]');zlabel('Depth [km]')
hold on

figure(1);
%--- CROSS SECTION  A-A'
%--- plot box location on maps 
subplot(2,2,2); hold on
...
figure(2);
plot3([(-box_l/2)*cos(phiA)-box_w*cos(phiB)+xshift (box_l/2)*cos(phiA)-box_w*cos(phiB)+xshift (box_l/2)*cos(phiA)+box_w*cos(phiB)+xshift (-box_l/2)*cos(phiA)+box_w*cos(phiB)+xshift (-box_l/2)*cos(phiA)-box_w*cos(phiB)+xshift],[(box_l/2)*sin(phiA)+box_w*sin(phiB)+yshift (-box_l/2)*sin(phiA)+box_w*sin(phiB)+yshift (-box_l/2)*sin(phiA)-box_w*sin(phiB)+yshift (box_l/2)*sin(phiA)-box_w*sin(phiB)+yshift (box_l/2)*sin(phiA)+box_w*sin(phiB)+yshift],zeros(1,5));
text(-box_l/2*cos(phiA)+xshift, box_l/2*sin(phiA)+yshift,0,'A'); 
text(box_l/2*cos(phiA)+xshift, -box_l/2*sin(phiA)+yshift,0,'A`'); 
figure(1);
%--- cross section 
...
figure(2);
plot3([(-box_l/2)*cos(phiA)-box_w*cos(phiB)+xshift (box_l/2)*cos(phiA)-box_w*cos(phiB)+xshift (box_l/2)*cos(phiA)+box_w*cos(phiB)+xshift (-box_l/2)*cos(phiA)+box_w*cos(phiB)+xshift (-box_l/2)*cos(phiA)-box_w*cos(phiB)+xshift],[(box_l/2)*sin(phiA)+box_w*sin(phiB)+yshift (-box_l/2)*sin(phiA)+box_w*sin(phiB)+yshift (-box_l/2)*sin(phiA)-box_w*sin(phiB)+yshift (box_l/2)*sin(phiA)-box_w*sin(phiB)+yshift (box_l/2)*sin(phiA)+box_w*sin(phiB)+yshift],zeros(1,5));
text(-box_l/2*cos(phiA)+xshift, box_l/2*sin(phiA)+yshift,0,'B'); 
text(box_l/2*cos(phiA)+xshift, -box_l/2*sin(phiA)+yshift,0,'B`'); 
figure(1);
...
print -f1 -r300 -dpng hypoDD_out.png
